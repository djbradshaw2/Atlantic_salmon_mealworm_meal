...
#snakemake -s '/home/microbiology/NICK/forwards.snakefile'  --configfile '/home/microbiology/NICK/forwards.json' --cores 4
...
rule annotate_rep_seqs:
    input:
      file=config['output_dir'] + "/denoising-stats.qzv",
      rep_seqs=config['output_dir'] + "/rep_seqs.qza",
      ref_reads=config["ref_reads"],
      ref_tax=config["ref_tax"]
    output:
      taxonomy=config['output_dir'] + "/taxonomy.qza"
    shell:
      "qiime feature-classifier classify-consensus-vsearch --i-reference-reads {input.ref_reads} --i-reference-taxonomy {input.ref_tax}  --i-query {input.rep_seqs} --o-classification {output.taxonomy} --p-threads 2 --verbose"
